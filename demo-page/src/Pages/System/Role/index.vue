<template>
  <div :class="pageName" style="height: 100%">
    ...
    <div class="header-buttons-bar" style="padding-left: 5px">
      <a-button type='primary' @click="showModal('add')" size="small">新增角色</a-button>
      ...
      <!--</a-button>-->
      <a-button @click="refresh" style="margin-left: 5px" size="small">刷新</a-button>
    </div>
    <!--高级搜索区域-->
      ...
      <div v-if="search.showAdvanced">
        <a-row>
          <a-col :lg="7" :md="12" :sm="24">
            <a-form-item label="角色名称" :labelCol="{ span: 6 }" :wrapperCol="{ span: 18 }">
              <a-input style="width:100%" size="small" placeholder="请输入角色名称" v-model="search.advancedForm.inputs[0]"
...
<script>
  ...
  const pageName = 'systemRole'
  const modalTitle = "角色"   //模态框的title标题通用变量

  const selOptions = ['']          //选择项所需要的配置，localstorage中的配置名称
  const selOptionMutation = 'INIT_SYSTEM_ROLE_OPTIONS'   //将选择项配置保存到store的mutation方法名
      ...
      return {
        ...
        table: {
          ...
          columns: [
            ...
            {title: '所属部门', dataIndex: 'departName', width: '180px', align: 'left', titleAlign: 'center'},
            {title: '角色说明', dataIndex: 'rolevalue', width: '300px', align: 'left', titleAlign: 'center'},
//            {title: '排序', dataIndex: 'sortNum', width: '120px', align: 'center', titleAlign: 'center'},
            {
              title: '操作',
              dataIndex: 'actions',
              width: '120px',
          ...
          scrollSize: {x: 950, y: window.innerHeight - 112},
//          rowSelection: {
//            selectedRowKeys: [],
...
//        debugger
    ...
    created(){
      ...
      reqRoleSsbm()
        .then((res)=>{
          if(res.success){
            this.modalOption.selectOptions.ssbm=res.data
          }
        })
...
//        this.modalOption.selectOptions['国民经济行业分类'].forEach(item=>item.isLeaf=false)
        ...
        const lsSearch = JSON.parse(localStorage.getItem('/asrsajjfixsearch'))['安监角色列表']
...
//------------------------------------------------------------------通用方法区域-----------------------------------------------------------------------
      ...
      toggleShowAdvancedSearch(){
        ...
        if (this.search.showAdvanced) {
          this.table.scrollSize.y = window.innerHeight - 150
...
//        this.search.advancedForm.tmlx &&this.search.advancedForm.tmlx!='' ? SelValues.push(`(a.tmlx = '${this.search.advancedForm.tmlx}') `): null
        if (this.search.advancedForm.departmentId) {
          this.search.advancedForm.departmentId != '' ? SelValues.push(`(a.__ddepartmentId = '${this.search.advancedForm.departmentId}') `) : null
        }
