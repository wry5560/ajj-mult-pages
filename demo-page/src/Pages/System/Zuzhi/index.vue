  ...
  <div :class="pageName" style="height: 100%">
    ...
    <div>
      ...
      >
        ...
        <template slot="zuzhiName" slot-scope="text,record,index">
    ...
    <div>
      ...
      >
        ...
        <template slot="footer" >
          <a-button v-show="modalOption.modelType" key="back" @click="modalCancel" size="small">返 回</a-button>
          <a-popconfirm title="您确认提交当前信息吗？" placement="topRight" okText="Yes" cancelText="No" @confirm="handleCommit">
            <a-button v-show="modalOption.modelType" key="submit" type="primary" :loading="modalOption.commitLoading"  size="small">提 交</a-button>
          </a-popconfirm>
        </template>
      </a-modal>
    </div>
...
//  import {GeneralQuerySelChildren} from '../api'
  ...
  const pageName='systemZuzhi'
  const modalTitle="组织"   //模态框的title标题通用变量

  const selOptions=['']          //选择项所需要的配置，localstorage中的配置名称
  const selOptionMutation='INIT_SYSTEM_ZUZHI_OPTIONS'   //将选择项配置保存到store的mutation方法名

  //修改以下获取store数据的getters 配置
  const getList='system_zuzhi_list'                //获取table的list
  const getSelOpitons='system_zuzhi_selOptions'   //获取选择项的配置内容
  const getDetailById='getZuzhiById'              //获取某一具体记录的详情

  //修改以下增删改查的Actions 方法名
  const reqList='reqZuzhiList'                   //查询列表
  const createAction='createZuzhi'             //新增记录
  const editAction='editZuzhi'                 //修改记录
  const delAction='delZuzhi'                   //删除
  const delMultiAction='delMutilZuzhi'                   //批量删除
...
//            {title: '序号', dataIndex: 'index', width: '50px',align: 'center'},
            {title: '组织名称',dataIndex: 'departname', width: '220px', align: 'left',titleAlign:'center', scopedSlots: {customRender: 'zuzhiName'}},
            {title: '组织简称', dataIndex: 'shortNm', width: '120px', align: 'left',titleAlign:'center'},
            {title: '描述', dataIndex: 'departDesc', width: '160px', align: 'left',titleAlign:'center'},
//            {title: '排序', dataIndex: 'sortNum', width: '50px', align: 'center',titleAlign:'center'},
            {title: '操作', dataIndex: 'actions', width: '100px', align: 'center', scopedSlots: {customRender: 'actionCell'}},
          ],
          size:'small',
          tableIsLoading:false,
          scrollSize: { x:700, y: window.innerHeight - 112},
           rowSelection:{
             selectedRowKeys: [],
...
//          start:this.pagination.reqData ? (this.pagination.current -1)*this.pagination.pageSize:0,
        ...
        this.$store.dispatch(reqList,parameter)
          .then((res)=>{
            ...
            this.table.dataSource=initTableChildren(
