...
<script>
  ...
  export default{
    ...
    methods:{
      ...mapGetters(['']),
      ...mapActions(['reqZuzhiData','reqWgfpList','editWgfp']),
      reqLeftTreeData(){
...
//          debugger
        ...
        if(parent) {
          ...
          this.initParent( this.wgData.find(i=>i.id==pId).pId)
        }
      },
...
//        console.log('onCheck info', info)
      ...
      saveWgfp(){
        ...
        selKeys.forEach(i=>{
          ...
          if(index ==-1){
            addWg.push(wgData.find(item=>item.id==i))
          }
        })
        authedWg.forEach(item=>{
          const index=selKeys.findIndex(i=>i==item.id)
          if(index ==-1)delWg.push(item)
        })
        const paramater2={
