<template>
  <div :class="pageNmae" style="height: 100%">
    ...
    <div>
      ...
      >
        <span slot="sfqyCell" slot-scope="text,record,index" >
           <a-badge :status="`${text==0 ? 'error':'success'}`" :text="`${text==0 ? '否':'是'}`"/>
        </span>
    ...
    <div>
      ...
      >
        ...
        <edit-form
          ...
          ref="commitForm"/>
        <jcx-table-form
          v-if="this.modalOption.modalType =='jcx'"
          @cancel="modalCancel"
          :recordId="modalOption.recordId"
          :tableHeight="modalTableHeight"
        />
        <qy-table-form
          v-if="this.modalOption.modalType =='qy'"
          @cancel="modalCancel"
          :recordId="modalOption.recordId"
          :tableHeight="modalTableHeight"
        />
...
<script>
  ...
  const modalTitle="检查计划"   //模态框的title标题
  ...
  const editAction='editZxjcplan'                 //修改记录
      ...
      return{
        ...
        table:{
          ...
          columns:[
            ...
            {title: '是否启用',dataIndex: 'sfqy', width: '80px', align: 'center',scopedSlots: {customRender: 'sfqyCell'}},
            {title: '计划名称', dataIndex: 'planname', width: '180px', align: 'center'},
            {title: '计划内容', dataIndex: 'neirong', width: '220px', align: 'center',},
            {title: '计划开始日期', dataIndex: 'planstartDate', width: '100px', align: 'center',},
            {title: '计划结束日期', dataIndex: 'planendDate', width: '100px',align: 'center'},
            {title: '所属组织', dataIndex: 'departName', width: '100px', align: 'center',},
            {title: '操作', dataIndex: 'actions', width: '250px', align: 'center', scopedSlots: {customRender: 'actionCell'}},
          ],
          size:'small',
