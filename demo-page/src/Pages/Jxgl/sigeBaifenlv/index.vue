...
//              item.index=index+(this.pagination.current -1)*this.pagination.pageSize+1
                ...
                item.sbl=item.dws==0? '- ' : ((item.wcs/item.dws).toFixed(4)) * 100 + '%'
                item.ccl =item.hmds==0? '- ' : (item.ccs/item.hmds).toFixed(4)* 100 + '%'
                item.jcl=item.lyhqys==0? '- ' : (item.jcs/item.lyhqys).toFixed(4)* 100 + '%'
                item.choucl=item.qtqys==0? '- ' : (item.choucs/item.qtqys).toFixed(4)* 100 + '%'
