      ...
      <a-row :gutter="16">
        ...
        <a-col :lg="12" :md="12" :sm="24">
          <a-form-item label="检查类型" :labelCol="{ span: 5 }" :wrapperCol="{ span: 19 }">
            <a-select size="small" placeholder="请选择检查类型"
                      v-decorator="['jclx',{rules: [{ required: true, message: '请选择检查类型', whitespace: true}],initialValue: initialValues.jclx}]" >
              <a-select-option v-for="(item) in selectOptions.jclx" :key="item.value":value="item.value">{{item.label}}</a-select-option>
            </a-select>
          </a-form-item>
        </a-col>
        <a-col :lg="24">
