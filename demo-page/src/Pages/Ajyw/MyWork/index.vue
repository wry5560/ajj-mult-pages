    ...
    <div>
      ...
      >
        <span slot="actionCell" slot-scope="text,record,index">
          <a v-if="activeTab=='2'" href="javascript:;" @click="showModal('query',record)">详情</a>
          ...
          <!--</a-popconfirm>-->
          <!--<a-divider v-if="" type="vertical"/>-->
          <a v-if="activeTab=='1'" href="javascript:;" @click="showModal('edit',record)">工作记录</a>
        </span>
      ...
      >
        ...
        <a-spin :spinning="modalLoading">
          ...
          <edit-form
            ...
            @cancel="modalCancel"
            :maxHeight="modalOption.bodyStyle['max-height']"
            ref="commitForm"/>
          <data-detail
        ...
        <template slot="footer">
          ...
          <a-popconfirm title="您确认提交当前信息吗？" placement="topRight" okText="Yes" cancelText="No" @confirm="handleCommit">
            <a-button v-show="modalOption.modelType =='jc'" key="submit" type="primary"
...
<script>
  ...
  const pageName = 'myWork'
  const modalTitle = "我的工作"   //模态框的title标题通用变量
  ...
  const getList = 'ajyw_mywork_list'                //获取table的list
  const getSelOpitons = ''   //获取选择项的配置内容
  const getDetailById = 'getMyWorkById'              //获取某一具体记录的详情

  //修改以下增删改查的Actions 方法名
  const reqList = 'reqMyWorkList'                   //查询列表
...
//            this.modalOption.width=''                             //修改弹出框的宽度
      ...
      reqTableData(filterOption){
        ...
        const parameter = {
          ...
          param1:this.activeTab=='1'?'1':'',
          param2:this.activeTab=='2'?'1':'',
        }
...
//              jsonData: JSON.stringify(values),
              ...
              param2:values.nycd,
              param3:values.fzr,
              param4:moment(values.jhwctime).format('YYYY-MM-DD')
            }
