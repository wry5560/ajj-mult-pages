  ...
  <div :class="pageName" style="height: 100%">
    ...
    <div class="header-buttons-bar" style="padding-left: 5px">
      ...
      <span style="display: inline-block;width: 260px">
        ...
        <a-tab-pane tab="已分配" key="2"></a-tab-pane>
        ...
        <a-row>
          <a-col :lg="7" :md="12" :sm="24">
            <a-form-item label="企业名称" :labelCol="{ span: 6 }" :wrapperCol="{ span: 18 }">
              <a-input style="width:100%" size="small" placeholder="请输入企业名称" v-model="search.advancedForm.inputs[0]"
          ...
          <a-col :lg="7" :md="12" :sm="24">
            <a-form-item label="注册地址" :labelCol="{ span: 6 }" :wrapperCol="{ span: 18 }">
              <a-input style="width:100%" size="small" placeholder="请输入检查依据" v-model="search.advancedForm.inputs[1]"
                       @pressEnter="onAdvancedSearch">
              </a-input>
            </a-form-item>
          </a-col>
          <a-col :lg="7" :md="12" :sm="24">
            <a-form-item label="企业类型" :labelCol="{ span: 6 }" :wrapperCol="{ span: 18 }">
              <a-cascader :options="modalOption.selectOptions['企业类型']" style="width:100%" size="small"
                          :loadData="selLoadData" placeholder="请选择企业类型" v-model="search.advancedForm.qylx"
                          changeOnSelect>
              </a-cascader>
            </a-form-item>
          </a-col>
          ...
          <a-col :lg="7" :md="12" :sm="24">
            <a-form-item label="国名经济类型" :labelCol="{ span: 6 }" :wrapperCol="{ span: 18 }">
              <a-cascader :options="modalOption.selectOptions['国民经济行业分类']" style="width:100%" size="small"
                          :loadData="selGmjjLoadData" placeholder="请选择国名经济类型" v-model="search.advancedForm.gmjjhyfl"
                          changeOnSelect>
              </a-cascader>
          ...
          <a-col :lg="7" :md="12" :sm="24">
            <a-form-item label="工贸行业类型" :labelCol="{ span: 6 }" :wrapperCol="{ span: 18 }">
              <a-select style="width:100%" size="small" placeholder="请选择工贸行业类型" v-model="search.advancedForm.jghy"
                        allowClear>
                <a-select-option v-for="(item) in modalOption.selectOptions['安全监管行业']" :key="item.value"
    ...
    <div>
      ...
      >
        ...
        <a-spin :spinning="modalLoading">
          ...
          <!--基础增删组件-->
          <edit-form
            v-if="modalOption.modelType =='add'||modalOption.modelType =='edit' || modalOption.modelType =='fenpei' "
...
<script>
  ...
  const pageName = 'arrageWork'
  const modalTitle = "工作分配"   //模态框的title标题通用变量

  const selOptions = ['nycd']          //选择项所需要的配置，localstorage中的配置名称
  const selOptionMutation = 'INIT_AJYW_ARRAGE_SELECTED_OPTIONS'   //将选择项配置保存到store的mutation方法名

  //修改以下获取store数据的getters 配置
  const getList = 'ajyw_arrage_list'                //获取table的list
  const getSelOpitons = 'ajyw_arrage_selOptions'   //获取选择项的配置内容
  const getDetailById = 'getArrageWorkById'              //获取某一具体记录的详情

  //修改以下增删改查的Actions 方法名
  const reqList = 'reqArrageWorkList'                   //查询列表
  const createAction = ''             //新增记录
  const editAction = 'fenpeiWork'      //修改记录
  const delAction = ''                   //删除
...
//            this.modalOption.width=''                             //修改弹出框的宽度
          ...
          case 'fenpei':
            ...
            this.modalOption.modelType = 'fenpei'
            this.modalOption.recordId = record.id
            this.modalOption.modalClass = 'nomal-modal'
            break
      ...
      reqTableData(filterOption){
        ...
        const parameter = {
          ...
          param2:this.activeTab=='1'?'1':'',
          param3:this.activeTab=='2'?'1':'',
        }
        ...
        this.$store.dispatch(reqList, parameter)
          .then((res) => {
            ...
            this.table.dataSource.forEach((item, index) => {
              ...
              item.jhwctime = item.jhwctime && item.jhwctime != '' ? moment(item.jhwctime).format('YYYY-MM-DD HH:ss') : ''
            })
...
//              jsonData: JSON.stringify(values),
              param1:this.modalOption.recordId,
              param2:values.nycd,
              param3:values.fzr,
              param4:moment(values.jhwctime).format('YYYY-MM-DD HH:ss')
            }
