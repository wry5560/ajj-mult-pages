<template>
  <div class="yingji_zj" style="height: 100%">
    <!--下面是顶部的按钮栏-->
    <div  class="header-buttons-bar" style="padding-left: 5px">
      <a-button type='primary' @click="showModal('add')"size="small">新增专家</a-button>
    ...
    data(){
      return{
        ...
        table:{
          dataSource:[],
          columns:[
            {title: '序号', dataIndex: 'id', width: '50px', key:'id',align: 'center',style:{'padding-left':'60px'}},
            ...
            {title: '毕业学校', dataIndex: 'byxx', width: '120px', key:'byxx',align: 'center',scopedSlots: {customRender: 'ellipsis'}},
...
//        debugger
    ...
    mounted(){
      ...
      this.$store.commit('INIT_ZJ_SELECTED_OPTIONS',[
        {name:'zbtype',value:ls['zbtype']},
      ])
      this.modalOption.selectOptions=this.yingji_zj_selOptions()
    },
    methods:{
      moment,
      ...mapGetters(['yingji_zj_list','yingji_zj_selOptions','getZjById']),
      ...mapActions(['reqZjList','createYjzj','editYjzj','delYjzj']),
        ...
        this.$refs.commitForm.form.validateFields((err, values) => {
          if (!err) {
            ...
            switch (this.modalOption.modelType) {
              case 'add':
                this.createYjzj(parameter).then((res)=>{
                  if (res.success==true){
                ...
                break
              case 'edit':
                this.editYjzj(parameter).then((res)=>{
                  if (res.success==true){
                    this.$message.success('提交成功！')
                    this.reqTableData()
                    setTimeout(()=>{
      ...
      deleteRowData(record){
        let parameter={
          jsonData:JSON.stringify(this.getZjById()(record.id)),
        }
      ...
      reqTableData(){
        ...
        this.reqZjList(parameter)
          .then((res)=>{
            this.table.dataSource=this.yingji_zj_list()
