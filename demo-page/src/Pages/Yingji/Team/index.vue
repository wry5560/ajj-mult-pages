<template>
  <div class="yingji_team" style="height: 100%">
    <!--下面是顶部的按钮栏-->
    <div  class="header-buttons-bar" style="padding-left: 5px">
      <a-button type='primary' @click="showModal('add')"size="small">新增应急队伍</a-button>
      ...
      return{
        ...
        table:{
          ...
          columns:[
            ...
            {title: '名称',dataIndex: 'dwname', width: '120px',key:'dwname', align: 'center',scopedSlots: {customRender: 'ellipsis'}},
...
//        debugger
    ...
    mounted(){
      ...
      this.$store.commit('INIT_TEAM_SELECTED_OPTIONS',[
        {name:'dwtype',value:ls['dwtype']},
      ])
      this.modalOption.selectOptions=this.yingji_Team_selOptions()
    },
    methods:{
        ...
        this.$refs.commitForm.form.validateFields((err, values) => {
          if (!err) {
            ...
            switch (this.modalOption.modelType) {
              case 'add':
                this.createTeam(parameter).then((res)=>{
                  if (res.success==true){
                ...
                break
              case 'edit':
                this.editTeam(parameter).then((res)=>{
                  if (res.success==true){
                    this.$message.success('提交成功！')
                    this.reqTableData()
                    setTimeout(()=>{
      ...
      deleteRowData(record){
        let parameter={
          jsonData:JSON.stringify(this.getTeamById()(record.id)),
        }
        this.table.tableIsLoading=true
        this.delTeam(parameter)
          .then((res)=>{
            if (res.success==true){
      ...
      reqTableData(){
        ...
        this.reqTeamList(parameter)
          .then((res)=>{
            this.table.dataSource=this.yingji_team_list()
