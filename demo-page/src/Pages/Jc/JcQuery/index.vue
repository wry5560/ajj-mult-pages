<template>
  <div class="jiancha-query" style="height: 100%">
    <!--下面是顶部的按钮栏-->
    ...
    <div>
      ...
      >
        <span slot="actionCell" slot-scope="text,record,index" >
          <a href="javascript:;" @click="showModal('jcYdDetail',record)">查看</a>
          <a-divider v-if="record.isprint!='1'" type="vertical" />
          <!--<a href="javascript:;" @click="showModal('map',record)">位置</a>-->
          <!--<a-divider v-if="" type="vertical" />-->
          <a v-if="record.isprint!='1'" href="javascript:;" @click="showModal('jc',record)">修改</a>
          <!--<a-divider v-if="" type="vertical" />-->
    ...
    <div>
      ...
      >
        <a-spin  :spinning="modalLoading">
          ...
          <jc-detail v-if="modalOption.modelType =='jcDetail'" :showTree="false" :jcType="modalOption.jcType"  @cancel="modalCancel" :departmentId="modalOption.recordId"  :recordId="modalOption.recordId" :height="modalOption.bodyStyle.height"/>
...
<script>
  ...
  const getList='jczf_jcjl_list'                //获取table的list
  const getSelOpitons='jczf_jc_selOptions'   //获取选择项的配置内容
  const getDetailById='getJcjlById'              //获取某一具体记录的详情

  //修改以下增删改查的Actions 方法名
  const reqList='reqJcjlList'                   //查询列表
      ...
      return{
        ...
        table:{
          ...
          columns:[
            ...
            {title: '检查人', dataIndex: 'jcusersname', width: '120px', align: 'left',titleAlign:'center'},
          ...
          scrollSize: { x:1850, y: window.innerHeight - 112},
//           rowSelection:{
//             selectedRowKeys: [],
    ...
    mounted(){
      this.$nextTick(function () {
        ...
        window.onresize = function(){
          ...
          _this.modalOption.bodyStyle.height=window.innerHeight-120 + 'px'
        }
        ...
        const lsSearch=JSON.parse(localStorage.getItem('/asrsajjfixsearch'))['执法列表']
...
//            this.modalOption.width=''                             //修改弹出框的宽度
          ...
          case 'jc':
            ...
            this.qyid=record.id
            this.modalOption.recordId=record.id
            this.modalOption.width='75%'                             //修改弹出框的宽度
          ...
          case 'jcYdDetail' :
            ...
            this.modalOption.departmentId=record.departmentId
            this.modalOption.recordId=record.id
            this.modalOption.bodyStyle.height=window.innerHeight-120 + 'px'
            this.qyid=record.id
            this.modalOption.width='70%'                             //修改弹出框的宽度
            this.modalOption.modalClass ='nomal-modal'
            break;
          case 'jcJdDetail':
            this.modalOption.title='企业检查详情'
        ...
        this.$store.dispatch(reqList,parameter)
          .then((res)=>{
            ...
            this.table.dataSource.forEach((item,index)=>{
              ...
              item.fctime=item.fctime &&item.fctime!='' ? moment(item.fctime).format('YYYY-MM-DD'):''
