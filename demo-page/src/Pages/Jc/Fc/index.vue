  ...
  <div :class="pageName" style="height: 100%">
    ...
    <div  class="header-buttons-bar" style="padding-left: 5px">
      ...
      <a-tabs @change="tabChange">
        ...
        <a-tab-pane tab="已复查" key="2"></a-tab-pane>
    ...
    <div>
      ...
      >
        <span slot="actionCell" slot-scope="text,record,index" >
          ...
          <a v-else href="javascript:;" @click="showModal('jc',record)">复查</a>
        </span>
    ...
    <div>
      ...
      >
        <a-spin  :spinning="modalLoading">
          ...
          <!--页面特殊组件-->
          <fc-form v-if="modalOption.modelType =='jc'" @cancel="modalCancel" :departmentId="modalOption.recordId" :qyid="qyid" :recordId="modalOption.recordId" :height="modalOption.bodyStyle.height" />

          <fc-detail v-if="modalOption.modelType =='query'" :jcType="modalOption.jcType"  @cancel="modalCancel" :departmentId="modalOption.recordId" :qyid="qyid" :recordId="modalOption.recordId" :height="modalOption.bodyStyle.height"/>
...
<script>
  ...
  //组件
  import fcForm from '../comps/fcForm.vue'
  import fcDetail from '../comps/fcDetail.vue'

  //页面全局变量

  const pageName='fucha'
  const modalTitle="复查"   //模态框的title标题通用变量
  ...
  const getList='jczf_fcjl_list'                //获取table的list
  const getSelOpitons='jczf_jc_selOptions'   //获取选择项的配置内容
  const getDetailById='getFcjlById'              //获取某一具体记录的详情

  //修改以下增删改查的Actions 方法名
  const reqList='reqFcjlList'                   //查询列表
...
//            {title: '企业类型', dataIndex: 'qylx0', width: '200px', align: 'left',titleAlign:'center'},
          ...
          scrollSize: { x:1100, y: window.innerHeight - 124},
//           rowSelection:{
//             selectedRowKeys: [],
    ...
    mounted(){
      this.$nextTick(function () {
        ...
        window.onresize = function(){
          _this.table.scrollSize.y=  _this.search.showAdvanced ? window.innerHeight - 204 :window.innerHeight - 124
          _this.modalOption.bodyStyle.height=window.innerHeight-80 + 'px'
        }
         document.getElementsByClassName('ant-table-body')[0].style.height=`${window.innerHeight -120}px`
        ...
        this.$store.dispatch(reqList,parameter)
          .then((res)=>{
            ...
            this.table.dataSource.forEach((item,index)=>{
              ...
              item.jctimeend=item.jctimeend &&item.jctimeend!='' ? moment(item.jctimeend).format('YYYY-MM-DD'):''
              item.yqzgsj=item.yqzgsj &&item.yqzgsj!='' ? moment(item.yqzgsj).format('YYYY-MM-DD'):''
              item.sffcwc=item.sffcwc=='1' ? '是':'否'
