...
<script>
  ...
  const pageName='meeting'
  const modalTitle="会议"   //模态框的title标题通用变量

  const selOptions=['']          //选择项所需要的配置，localstorage中的配置名称
  const selOptionMutation='INIT_MEETING_SELECTED_OPTIONS'   //将选择项配置保存到store的mutation方法名

  //修改以下获取store数据的getters 配置
  const getList='meeting_list'                //获取table的list
  const getSelOpitons='meeting_selOptions'   //获取选择项的配置内容
  const getDetailById='getMeetingById'              //获取某一具体记录的详情

  //修改以下增删改查的Actions 方法名
  const reqList='reqMeethinList'                   //查询列表
  const createAction='createHy'             //新增记录
  const editAction='editHy'                 //修改记录
  const delAction='delHy'                   //删除
      ...
      return{
        ...
        table:{
          ...
          columns:[
            ...
            {title: '会议状态', dataIndex: 'state', width: '60px', align: 'center',titleAlign:'center'},
//            {title: '所属组织', dataIndex: 'departmentid', width: '80px', align: 'center',titleAlign:'center'},
            {title: '操作', dataIndex: 'actions', width: '120px', align: 'center', scopedSlots: {customRender: 'actionCell'}},
          ],
          size:'small',
          tableIsLoading:false,
          scrollSize: { x:1250, y: window.innerHeight - 112},
//           rowSelection:{
//             selectedRowKeys: [],
        ...
        this.$store.dispatch(reqList,parameter)
          .then((res)=>{
            ...
            this.table.dataSource.forEach((item,index)=>{
              ...
              item.hytime=item.hytime && item.hytime!='' ? moment(item.hytime).format('YYYY-MM-DD HH:mm'):''
