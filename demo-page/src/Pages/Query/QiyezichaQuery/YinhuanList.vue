<template>
  <div>
    ...
    >
      ...
      <span slot="jjcd" slot-scope="text,record,index" >
          <span v-if="text=='1'">一般隐患</span>
          <span v-if="text=='3'">重大隐患</span>
        </span>
      <span slot="sfwc" slot-scope="text,record,index" >
...
//  const modalTitle="检查"   //模态框的title标题通用变量
  ...
  const getList='query_qiyezicha_yhlist'                //获取table的list
  ...
  const reqList='reqQiyeJcYinhuanList'                   //查询列表

  export default {
    name:'queryZichaYinhuanList',
    components:{
//      jcDetail
    },
    data(){
      return{
        ...
        table:{
          ...
          columns:[
            ...
            {title: '隐患内容',dataIndex: 'gdNm', width: '250px', align: 'left',titleAlign:'center'},
...
//            {title: '隐患数/整改数', dataIndex: 'yhzg', width: '120px', align: 'center',titleAlign:'center'},
            {title: '隐患级别', dataIndex: 'jjcd', width: '100px', align: 'center',titleAlign:'center', scopedSlots: {customRender: 'jjcd'}},
//            {title: '状态', dataIndex: 'state', width: '100px', align: 'left',titleAlign:'center'},
            {title: '负责人', dataIndex: 'fzr', width: '100px', align: 'center',titleAlign:'center'},
            {title: '解决措施', dataIndex: 'jjcs', width: '100px', align: 'center',titleAlign:'center'},
            {title: '是否完成', dataIndex: 'sfwc', width: '100px', align: 'center',titleAlign:'center', scopedSlots: {customRender: 'sfwc'}},
...
//        item.yhzg=item.yhnum ? item.yhnum + '/'+ item.yzgnum :'- / -'
      ...
      this.pagination.total=this.query_qiyezicha_yhTotalCount()

      this.table.columns=initColumn(this.table.columns)
    },
    methods:{
      moment,
      ...mapGetters(['getQiyeZiChaSearchValues','query_qiyezicha_yhTotalCount']),
      ...mapActions(['','']),
