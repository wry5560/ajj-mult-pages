...
const teshuzuoye = {
  ...
  mutations: {
    INIT_TSZY_LIST: (state, payload) => {
      state.tszyList=payload
    },
  },
  actions: {
    // //查询特殊作业列表
    reqTszyList:({commit},params)=>{
        ...
        GeneralQuery(parameter)
          .then((res)=>{
            if(res.success){
              ...
              commit('INIT_TSZY_LIST',res.data)
              resolve(res)
            }else{
  ...
  getters:{
    ...
    get_tszy_list:(state) => {
      state.tszyList.forEach((item,index)=>{
        item.index=index+1
        // item.departName=item.__ddepartmentId.departName
      })
      return state.tszyList
    },
